"""
OCTIP script for splitting an OCT dataset into train, validation and test subsets.
"""
...
def save_ground_truth(output_file_name,
    ...
    with open(output_file_name, 'w') as f:
        ...
        for row in range(len(patient_ids)):
            if patient_ids[row] in selected_patient_ids:
                f.write(','.join([str(x) for x in data.loc[row, :].values]) + '\n')


def main():
    """
    Splits an OCT dataset into train, validation and test subsets.
    """

    # parsing the command line
    parser = ArgumentParser(
        description = 'Splits an OCT dataset into train, validation and test subsets.')
    parser.add_argument('-t', '--ground_truth', required = True,
    ...
    parser.add_argument('-p', '--patient_col', default = 'patient',
                        help = 'column containing the patient identifiers in the ground truth file')
    if len(sys.argv[1:]) == 0:
        parser.print_usage()
        parser.exit()
    ...
    # per-patient label histograms
    data = pd.read_csv(args.ground_truth)
    patient_ids = data[args.patient_col]
    ...
    save_ground_truth('train.csv', train_ids, patient_ids, data)
    save_ground_truth('validation.csv', validation_ids, patient_ids, data)
    save_ground_truth('test.csv', test_ids, patient_ids, data)
